...
void Profiler::OutputMap( CollectiveCommunication& comm, InfoContainerMap& run_infos_map, const double scale_factor )
{
	...
	BOOST_FOREACH( typename InfoContainerMap::value_type el, run_infos_map  ) {
		OutputCommon( comm, el.second, (boost::format("%s/prof_p%d_ref%s.csv") % dir % comm.size() % el.first).str(), scale_factor );
	}
}
...
namespace Stuff {
	...
	struct ProgressiveWeights {
		const int prog_;
		ProgressiveWeights(const int prog):prog_(prog){}
		double apply (const double to_weigh, const int /*current*/, const int /*max*/) {
	...
	class LoopTimer {
		...
		public:
			LoopTimer(CounterType& counter, const int iteration_count, OutputStreamType& output_stream = std::cout, WeightType weight = WeightType() )
				: counter_(counter),
