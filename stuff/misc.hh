 ...
 **/
template< class FieldMatrixImp >
FieldMatrixImp transposeMatrix( const FieldMatrixImp& matrix,
                                const FieldMatrixImp& transpose )
{
    assert( matrix.rowdim() == transpose.coldim() );
    FieldMatrixImp ret( 0.0 );
    typedef typename FieldMatrixImp::RowIterator
        RowIteratorType;
    typedef typename FieldMatrixImp::ConstRowIterator
        ConstRowIteratorType;
    ConstRowIteratorType itEnd = matrix.end();
    RowIteratorType retIt = ret.begin();
    for (   ConstRowIteratorType it = matrix.begin();
            it != itEnd;
            ++it ) {
