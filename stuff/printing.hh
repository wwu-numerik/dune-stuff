...
/** \brief print a SparseRowMatrix (or any interface conforming object) to a given stream in matlab (laodable-) format
  ...
  **/
template < class T, class stream >
void printSparseRowMatrixMatlabStyle( const T& arg, std::string name, stream& out, const double eps = Parameters().getParam( "eps", 1e-14) )
{
...
void printISTLMatrixMatlabStyle( const MatrixType& arg, std::string name, stream& out,  const double eps = Parameters().getParam( "eps", 1e-14) )
{
                ...
                for( int i = 0; i < block.N(); ++ i ) {
                    for( int j = 0; j < block.M(); ++ j ) {
                        ...
                        if ( std::fabs(value) > eps ) {
                            ...
                            out << name << "(" << real_row << "," << real_col << ")=" << std::setprecision( matlab_output_precision ) << value << ";\n";
                        }
                    }
                }
            }
