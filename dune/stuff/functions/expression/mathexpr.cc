...
int operator==(const ROperation& op, const double v) { return (op.op == Num && op.ValC == v); }
...
ROperation ROperation::operator+() const { return *this; }
...
int operator==(const RVar& var1, const RVar& var2) { return (var1.pval == var2.pval && EqStr(var1.name, var2.name)); }
...
void ArcSinus(double*& p) { *p = ((*p == ErrVal || fabsl(*p) > 1) ? ErrVal : asinl(*p)); }
void ArcCosinus(double*& p) { *p = ((*p == ErrVal || fabsl(*p) > 1) ? ErrVal : acosl(*p)); }
void ArcTangente(double*& p) { *p = ((*p == ErrVal) ? ErrVal : atanl(*p)); }
void Logarithme(double*& p) { *p = ((*p == ErrVal || *p <= 0) ? ErrVal : logl(*p)); }
void Exponentielle(double*& p) { *p = ((*p == ErrVal || *p > DBL_MAX_EXP) ? ErrVal : expl(*p)); }
void Sinus(double*& p) { *p = ((*p == ErrVal || fabsl(*p) > inveps) ? ErrVal : sinl(*p)); }
void Tangente(double*& p) { *p = ((*p == ErrVal || fabsl(*p) > inveps) ? ErrVal : tanl(*p)); }
void Cosinus(double*& p) { *p = ((*p == ErrVal || fabsl(*p) > inveps) ? ErrVal : cosl(*p)); }
void Racine(double*& p) { *p = ((*p == ErrVal || *p > sqrtmaxfloat || *p < 0) ? ErrVal : sqrtl(*p)); }
