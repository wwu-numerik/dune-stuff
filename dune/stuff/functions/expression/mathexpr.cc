...
{
  op = ROp.op; pvar = ROp.pvar; pvarval = ROp.pvarval; ValC = ROp.ValC; pfunc = ROp.pfunc; containfuncflag = 0;
  pinstr = NULL; pvals = NULL; ppile = NULL; pfuncpile = NULL;
...
ROperation::ROperation(double x)
{
  ...
  pvar = NULL; pvarval = NULL; pfunc = NULL; containfuncflag = 0; pinstr = NULL; pvals = NULL; ppile = NULL;
  pfuncpile = NULL;
  BuildCode();
}
...
int operator==(const ROperation& op1, const ROperation& op2)
{
  ...
  if (op1.op == Fun)return (op1.pfunc ==
                              op2.pfunc); // *op1.pfunc==*op2.pfunc could imply infinite loops in cases of self-dependence
...
{
  int i;
  for (i = 0; i < (int)strlen(s); i++)if (!IsNumeric(s[i]))return 0; return 1;
}
...
void IsolateVars(char*& s, int nvar, PRVar* ppvar, int nfunc, PRFunction* ppfunc) //Deletes the old string
{
  ...
  for (i = 0; s[i]; i++) {
    ...
    if (((j = IsVar(s, i, nvar, ppvar)) > IsFunction(s, i, nfunc, ppfunc)) || ((CompStr(s, i, "pi") || CompStr(s, i, "PI")
                                                                                || CompStr(s, i, "Pi")) && (j = 2))) {
...
{
  ValC = ErrVal; mmb1 = NULL; mmb2 = NULL; pvar = NULL; op = ErrOp; pvarval = NULL; containfuncflag = 0; pfunc = NULL;
  pinstr = NULL; pvals = NULL; ppile = NULL; pfuncpile = NULL;
  ...
  if (s[0] == '(' && SearchCorOpenbracket(s, 0) == (int)strlen(s) - 1) {
    if (CompStr(s, 1, "exp")) {
      ...
      for (i = -1, k = 0, j = 0; j < nfuncp; j++)if (CompStr(s, 1, ppfuncp[j]->name) && k < (int)strlen(ppfuncp[j]->name)) {
    ...
    if (op == Fun)if (mmb2->NMembers() != pfunc->nvars) {
        op = ErrOp;
        mmb1 = NULL;
        mmb2 = NULL;
        goto fin;
...
signed char ROperation::ContainFunc(const RFunction& func) const // Recursive test on subfunctions
{
  ...
  containfuncflag = 1;
  if (op == Fun)if (pfunc->op.ContainFunc(func)) {
      containfuncflag = 0;
      return 1;
    }
...
ROperation ROperation::Diff(const RVar& var) const
{
  ...
  case Div: if (mmb2->ContainVar(var))return (((*mmb2) * (mmb1->Diff(var)) - (*mmb1) * (mmb2->Diff(var))) / ((
                                                                                                                   *mmb2) ^ 2));
  ...
  case Atan:
    ...
    else return ((mmb2->NthMember(1).Diff(var)) * (mmb2->NthMember(2)) - (mmb2->NthMember(2).Diff(var)) * (mmb2->NthMember(
                                                                                                               1))) / (((mmb2->NthMember(1)) ^ 2) + ((mmb2->NthMember(2)) ^ 2));
...
char* ROperation::Expr() const
{
  ...
  case Add:
    ...
    else if (f)sprintf(s, "(%s)+%s", s1, s2);
    else if (g)sprintf(s, "%s+(%s)", s1, s2);
    else
      sprintf(s, "%s+%s", s1, s2);
    break;
  case Sub:
  ...
  case E10:
    ...
    else if (f)sprintf(s, "(%s)E%s", s1, s2);
    else if (g)sprintf(s, "%sE(%s)", s1, s2);
    else
...
double ROperation::Val() const
{
  ...
  for (; *p1 != NULL; p1++)
    if (*p1 == &NextVal)*(++p3) = **(p2++);
    else if (*p1 == &RFunc) ApplyRFunc(*(p4++), p3);
    else (**p1)(p3);
  return *p3;
}
...
void ROperation::BuildCode()
{
  ...
  case Juxt: BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr,
    ...
    break;
  case Add: BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr,
                       pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile,
                       &Addition);
    break;
  case Sub: BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr,
                       pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile,
                       &Soustraction);
    break;
  case Mult: BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr,
                        pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile,
                        &Multiplication);
    break;
  case Div: BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr,
                       pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile,
                       &Division);
    break;
  case Pow: BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr,
                       pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile,
                       &Puissance);
    break;
  case NthRoot: BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr,
                           pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile,
                           &RacineN);
    break;
  case E10: BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr,
                       pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile,
                       &Puiss10);
    break;
  case Opp: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                       ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Oppose);
    break;
  case Sin: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                       ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Sinus);
    break;
  case Sqrt: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                        ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Racine);
    break;
  case Ln: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                      ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Logarithme);
    break;
  case Exp: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                       ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Exponentielle);
    break;
  case Cos: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                       ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Cosinus);
    break;
  case Tg: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                      ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Tangente);
    break;
  case Atan: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                        ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, (mmb2->NMembers() > 1 ? &ArcTangente2 : &ArcTangente));
    break;
  case Asin: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                        ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &ArcSinus);
    break;
  case Acos: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                        ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &ArcCosinus);
    break;
  case Abs: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
                       ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Absolu);
    break;
  case Fun: BCFun(pinstr, mmb2->pinstr, pvals, mmb2->pvals, ppile,
                    mmb2->ppile, pfuncpile, mmb2->pfuncpile, pfunc);
    break;
  default: BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals,
