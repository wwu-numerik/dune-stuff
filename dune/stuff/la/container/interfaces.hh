  ...
  static size_t assert_is_size_t_compatible_and_convert(const SignedSizeType& size)
  {
    ...
    if (UnsignedSizeType(size) > std::numeric_limits< size_t >::max())
      DUNE_THROW_COLORFULLY(Exceptions::index_out_of_range,
                            "Given size (" << size << ") is too large for size_t (max "
...
public:
  ...
  /**
   * \defgroup python_bindings ´´These methods are necesarry for the python bindings. They are provided by the interface!``
   * \{
   */
  ...
  {
    const auto max = amax();
    if (!(max.first >= std::numeric_limits< ScalarType >::min()
          && max.first <= std::numeric_limits< ScalarType >::max()))
      DUNE_THROW_COLORFULLY(Exceptions::index_out_of_range,
                            "The index (" << max.first << ") does not fit into ScalarType (min "
                            << std::numeric_limits< DUNE_STUFF_SSIZE_T >::min() << ", max "
...
public:
  ...
  {
    add_to_entry(this->assert_is_size_t_compatible_and_convert(ii),
  ...
  {
    set_entry(this->assert_is_size_t_compatible_and_convert(ii),
              this->assert_is_size_t_compatible_and_convert(jj),
              value);
  } // ... pb_set_entry(...)
   ...
   * \{
   */
  inline void DUNE_DEPRECATED_MSG("Please use add_to_entry()!") add(const size_t ii,
                                                                    const size_t jj,
  ...
  inline void DUNE_DEPRECATED_MSG("Please use set_entry()!") set(const size_t ii,
                                                                 const size_t jj,
                                                                 const ScalarType& value)
  {
