...
public:
  ...
  {
    return {"qr.colpivhouseholder", "qr.fullpivhouseholder", "qr.householder", "lu.fullpiv", "llt", "ldlt",
            "lu.partialpiv"};
  }
  ...
  size_t apply(const EigenMappedDenseVector< S >& rhs,
  ...
  size_t apply(const EigenDenseVector< S >& rhs,
               EigenMappedDenseVector< S >& solution,
...
//           , "llt.cholmodsupernodal" // <- does not compile
           ...
           , "superlu"                 // <- untested
#endif
           , "cg.diagonal.lower"       // <- does only work with symmetric matrices, may produce correct results but takes a lot of memory
           , "cg.diagonal.upper"       // <- does only work with symmetric matrices, may produce correct results but takes a lot of memory
           , "cg.identity.lower"       // <- does only work with symmetric matrices, may produce correct results but takes a lot of memory
           , "cg.identity.upper"       // <- does only work with symmetric matrices, may produce correct results but takes a lot of memory
    };
  }
  ...
  static Common::ConfigTree options(const std::string& type)
  {
    ...
    if (type == "lu.sparse" || type == "qr.sparse" || type == "ldlt.simplicial" || type == "llt.simplicial"
        || type == "lu.umfpack" || type == "spqr" || type == "llt.cholmodsupernodal" || type == "superlu")
