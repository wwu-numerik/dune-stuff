...
public:
  ...
  void apply(const IstlDenseVector< S >& rhs, IstlDenseVector< S >& solution, const Common::ConfigTree& opts) const
  {
    ...
    if (type == "bicgstab.amg.ilu0") {
      ...
      Amg::Parameters criterion_parameters(opts.get("criterion.max_level", default_opts.get< size_t >("criterion.max_level")),
                                          opts.get("criterion.coarse_target",
                                                   default_opts.get< size_t >("criterion.coarse_target")),
                                          opts.get("criterion.min_coarse_rate",
                                                   default_opts.get< S >("criterion.min_coarse_rate")),
      ...
      criterion_parameters.setDefaultValuesAnisotropic(opts.get("criterion.anisotropy_dim",
                                                               default_opts.get< size_t >("criterion.anisotropy_dim"))); // <- dim
      criterion_parameters.setDebugLevel(opts.get("criterion.verbose",
                                                  default_opts.get< size_t >("criterion.verbose")));
