...
class Solver< IstlRowMajorSparseMatrix< S >, GridView >
...
public:
  ...
  Solver(const MatrixType& matrix,
         const GridView* grid_view = nullptr)
    : matrix_(matrix)
  ...
  void apply(const IstlDenseVector< S >& rhs, IstlDenseVector< S >& solution, const Common::ConfigTree& opts) const
  {
    ...
    if (type == "bicgstab.ilut") {
      ...
      // scalar product
#if HAVE_MPI
      Dune::OverlappingSchwarzScalarProduct< typename IstlDenseVector< S >::BackendType,
                                             CommunicationType > scalar_product(communication);
#else // HAVE_MPI
      ...
      typedef OverlappingSchwarzOperator< typename MatrixType::BackendType,
                             DomainType,
                             DomainType,
                             CommType> MatrixOperatorType;
      MatrixOperatorType matrix_operator(matrix_.backend(), comm);
      typedef ParSSOR< typename MatrixType::BackendType,
                       DomainType,
                       DomainType,
                       CommType> PreconditionerType;
      PreconditionerType preconditioner(matrix_.backend(),
                                        opts.get("preconditioner.iterations",
                                                 default_opts.get< size_t >("preconditioner.iterations")),
                                        opts.get("preconditioner.relaxation_factor",
