...
public:
                             ...
                             const std::string& smoother_type)
  {
    ...
    smoother_parameters_ILU.relaxationFactor = opts.get("smoother.relaxation_factor",
    ...
    smoother_parameters_SSOR.iterations = opts.get("smoother.iterations",
                                                   default_opts.get< size_t >("smoother.iterations"));
    smoother_parameters_SSOR.relaxationFactor = opts.get("smoother.relaxation_factor",
                                                         default_opts.get< S >("smoother.relaxation_factor"));
    ...
    if (smoother_type == "ilu0") {
      ...
      SequentialPreconditioner seq_preconditioner;
       BlockPreconditioner< IstlVectorType, IstlVectorType,
          CommunicatorType,
          SequentialPreconditioner > preconditioner(seq_preconditioner, communicator_);
      // define the BiCGStab as the actual solver
      BiCGSTABSolver< IstlVectorType > solver(matrix_operator,
                                              scalar_product,
                                              preconditioner,
