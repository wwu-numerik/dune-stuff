...
void ensureParamFile(std::string filename)
{
  ...
  if (!boost::filesystem::exists(filename)) {
    ...
    file << "filename = /home/felix/Projects/dune/dune-lrbms/dune-rb/dune/rb/grid/examples/data/johansen_formation.grdecl" << std::endl;
...
void measureTiming(GridProviderType& gridProvider)
{
  ...
  std::cout << "  wrapped grid:     " << timer.elapsed() << " sec, " << numWrappedGridElements << " elements" << std::endl;
  timer.reset();
  ...
  std::cout << "  local grid:       " << timer.elapsed() << " sec, " << numLocalGridElements << " elements" << std::endl;
  timer.reset();
}
