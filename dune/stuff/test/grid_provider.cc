  ...
  : public testing::Test
{
  ...
  static void fulfills_interface(GridProviderType& grid_provider)
  {
    ...
    typename GridProviderType::template Leaf< ChoosePartView::view >::Type
        DUNE_UNUSED(leaf_view_auto) = grid_provider.template leaf< ChoosePartView::view >();
    typename GridProviderType::LeafGridViewType DUNE_UNUSED(leaf_view) = grid_provider.leaf_view();
#if HAVE_DUNE_FEM
    typename GridProviderType::template Leaf< ChoosePartView::part >::Type
        DUNE_UNUSED(leaf_part_auto) = grid_provider.template leaf< ChoosePartView::part >();
    typename GridProviderType::LeafGridPartType leaf_part = grid_provider.leaf_part();
#endif // HAVE_DUNE_FEM
    for (int level = 0; level <= grid.maxLevel(); ++level) {
      typename GridProviderType::template Level< ChoosePartView::view >::Type
          DUNE_UNUSED(level_view_auto) = grid_provider.template level< ChoosePartView::view >(level);
      typename GridProviderType::LevelGridViewType DUNE_UNUSED(level_view) = grid_provider.level_view(level);
#if HAVE_DUNE_FEM
      typename GridProviderType::template Level< ChoosePartView::part >::Type
          DUNE_UNUSED(level_part_auto) = grid_provider.template level< ChoosePartView::part >(level);
