  ...
  : public testing::Test
{
  ...
  static void fulfills_interface(GridProviderType& grid_provider)
  {
    ...
    std::shared_ptr< typename GridProviderType::template Leaf< Grids::ChoosePartView::view >::Type >
        leaf_view_1 = grid_provider.template leaf< Grids::ChoosePartView::view >();
    std::shared_ptr< typename GridProviderType::LeafGridViewType > leaf_view_2 = grid_provider.leaf_view();
#if HAVE_DUNE_FEM
    std::shared_ptr< typename GridProviderType::template Leaf< Grids::ChoosePartView::part >::Type >
        leaf_part_1 = grid_provider.template leaf< Grids::ChoosePartView::part >();
    std::shared_ptr< typename GridProviderType::LeafGridPartType > leaf_part_2 = grid_provider.leaf_part();
#endif // HAVE_DUNE_FEM
    for (int level = 0; level <= grid->maxLevel(); ++level) {
      std::shared_ptr< typename GridProviderType::template Level< Grids::ChoosePartView::view >::Type >
          level_view_1 = grid_provider.template level< Grids::ChoosePartView::view >(level);
      std::shared_ptr< typename GridProviderType::LevelGridViewType > level_view_2 = grid_provider.level_view(level);
#if HAVE_DUNE_FEM
      std::shared_ptr< typename GridProviderType::template Level< Grids::ChoosePartView::part >::Type >
          level_part_1 = grid_provider.template level< Grids::ChoosePartView::part >(level);
