  ...
  : public ::testing::Test
{
  void fulfills_interface() const
  {
    ...
    EXPECT_TRUE(d_amax.first == 0 && Dune::FloatCmp::eq(d_amax.second, D_ScalarType(1)))
        << d_amax.first << ",  " << d_amax.second;
  ...
  void produces_correct_results() const
  {
    ...
    EXPECT_TRUE(amax.first == 0 && Dune::FloatCmp::eq(amax.second, ScalarType(0)))
        << amax.first << " , " << amax.second;
    amax = ones.amax();
    EXPECT_TRUE(amax.first == 0 && Dune::FloatCmp::eq(amax.second, ScalarType(1)))
        << amax.first << " , " << amax.second;
    amax = countingup.amax();
    EXPECT_TRUE(amax.first == 3 && Dune::FloatCmp::eq(amax.second, ScalarType(3)))
        << amax.first << " , " << amax.second;
    amax = testvector_1.amax();
    EXPECT_TRUE(amax.first == 1 && Dune::FloatCmp::eq(amax.second, ScalarType(2)))
        << amax.first << " , " << amax.second;
    ...
    EXPECT_TRUE(amax.first == 1 && Dune::FloatCmp::eq(amax.second, ScalarType(3)))
        << amax.first << " , " << amax.second;
    amax = testvector_5.amax();
    EXPECT_TRUE(amax.first == 3 && Dune::FloatCmp::eq(amax.second, ScalarType(3.5)))
        << amax.first << " , " << amax.second;
    ...
    EXPECT_TRUE(diff_operator_iminus == diff_operator_minus && diff_sub_1 == diff_sub_2 && diff_isub == diff_sub_1
                && diff_sub_1 == diff_operator_minus && diff_sub_1 == diff_correct)
  ...
  : public ::testing::Test
{
  ...
  void fulfills_interface() const
  {
    ...
    for (size_t ii = 0; ii < d_rows; ++ii) {
      for (size_t jj = 0; jj < d_cols; ++jj) {
        ...
        EXPECT_FALSE(FloatCmp::ne(d_by_size_and_pattern.get_entry(ii, jj),
  ...
  void produces_correct_results() const
  {
    ...
    EXPECT_TRUE(result_mv_1 == vector_zeros && result_mv_2 == vector_zeros)
        << result_mv_1 << " vs " << result_mv_2 << " vs. " << vector_zeros;
    ...
    EXPECT_TRUE(FloatCmp::eq(result_mv_1[0], ScalarType(1.25)) && FloatCmp::eq(result_mv_1[1], ScalarType(1.25))
        && FloatCmp::eq(result_mv_1[2], ScalarType(1.75)) && FloatCmp::eq(result_mv_1[3], ScalarType(0))) << result_mv_1;
    ...
    EXPECT_TRUE(FloatCmp::eq(result_mv_1[0], ScalarType(5)) && FloatCmp::eq(result_mv_1[1], ScalarType(6))
        && FloatCmp::eq(result_mv_1[2], ScalarType(7)) && FloatCmp::eq(result_mv_1[3], ScalarType(8))) << result_mv_1;
    ...
    EXPECT_TRUE(FloatCmp::eq(result_mv_1[0], ScalarType(0.5)) && FloatCmp::eq(result_mv_1[1], ScalarType(2.5))
        && FloatCmp::eq(result_mv_1[2], ScalarType(-0.5)) && FloatCmp::eq(result_mv_1[3], ScalarType(0))) << result_mv_1;
    ...
    for (size_t ii = 0; ii < rows ; ++ii) {
      for (size_t jj = 0; jj < cols; ++jj) {
        EXPECT_FALSE(FloatCmp::ne(scaled.get_entry(ii, jj), testmatrix_1.get_entry(ii, jj)*ScalarType(10))
    ...
    for (size_t ii = 0; ii < rows ; ++ii) {
      for (size_t jj = 0; jj < cols; ++jj) {
        EXPECT_FALSE(FloatCmp::ne(result_axpy.get_entry(ii, jj), ScalarType(1.5)))
            << result_axpy.get_entry(ii, jj) << " vs. " << ScalarType(1.5);
      }
    }
    ...
    for (size_t ii = 0; ii < rows ; ++ii) {
      for (size_t jj = 0; jj < cols; ++jj) {
        EXPECT_FALSE(FloatCmp::ne(result_axpy.get_entry(ii, jj), ScalarType(0)))
            << result_axpy.get_entry(ii, jj) << " vs. " << ScalarType(0);
      }
    }
