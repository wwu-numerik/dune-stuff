  ...
  void produces_correct_results() const
  {
    ...
    for (size_t ii = 0; ii < dim; ++ii) {
      if (!Dune::FloatCmp::eq(scaled[ii], ScalarType(2)*testvector_1[ii])
        ...
        DUNE_THROW_COLORFULLY(Dune::Exception, scaled[ii] << " vs. "<< scaled_operator[ii] << " vs." << testvector_1[ii]);
    }
    scaled = testvector_3;
    scaled.scal(ScalarType(-2));
    ...
    for (size_t ii = 0; ii < dim; ++ii) {
      if (!Dune::FloatCmp::eq(scaled[ii], ScalarType(-2)*testvector_3[ii])
        ...
        DUNE_THROW_COLORFULLY(Dune::Exception, scaled[ii] << " vs. "<< scaled_operator[ii] << " vs." << testvector_3[ii]);
    }
    scaled = countingup;
    ...
    for (size_t ii = 0; ii < dim; ++ii) {
      if (!Dune::FloatCmp::eq(scaled[ii], ScalarType(2.2)*countingup[ii])
        ...
        DUNE_THROW_COLORFULLY(Dune::Exception, scaled[ii] << " vs. "<< scaled_operator[ii] << " vs." << countingup[ii]);
    }
    scaled = testvector_5;
    ...
    for (size_t ii = 0; ii < dim; ++ii) {
      if (!Dune::FloatCmp::eq(scaled[ii], ScalarType(-3.75)*testvector_5[ii])
        ...
        DUNE_THROW_COLORFULLY(Dune::Exception, scaled[ii] << " vs. "<< scaled_operator[ii] << " vs." << testvector_5[ii]);
    }
  ...
  : public ::testing::Test
{
  ...
  void produces_correct_results() const
  {
    ...
    for (size_t ii = 0; ii < dim; ++ii) {                                                                // |1, 2, 3, 4|
      for (size_t jj = 0; jj < dim; ++jj)                                                                // |2, 3, 4, 5|
        matrix_ones.set_entry(ii, jj, ScalarType(-0.5)*ScalarType(ii) + ScalarType(1.5)*ScalarType(jj)); // |3, 4, 5, 6|
    }
...
struct DenseMatrixTest
  : public MatrixTestBase< DenseMatrixVectorCombination >
{
...
struct SparseMatrixTest
  : public MatrixTestBase< SparseMatrixVectorCombination >
{
