  ...
  : public ::testing::Test
{
  void fulfills_interface() const
  {
    ...
    if (!d_by_size.has_equal_shape(d_deep_copy)) DUNE_THROW(Dune::Exception, "");
    // * of the container as the interface
  ...
  : public ::testing::Test
{
  void fulfills_interface() const
  {
    ...
    if (d_size != dim) DUNE_THROW(Dune::Exception, d_size << " vs. " << dim);
    ...
    if (d_dim != dim) DUNE_THROW(Dune::Exception, d_dim << " vs. " << dim);
    bool d_almost_equal = d_by_size.almost_equal(d_by_size);
    if (!d_almost_equal) DUNE_THROW(Dune::Exception, "");
    d_by_size_and_value.scal(D_ScalarType(0));
    if (!d_by_size_and_value.almost_equal(d_by_size)) DUNE_THROW(Dune::Exception, "");
    D_ScalarType d_dot = d_by_size.dot(d_by_size_and_value);
    if (!Dune::FloatCmp::eq(d_dot, D_ScalarType(0))) DUNE_THROW(Dune::Exception, d_dot);
    D_ScalarType d_l1_norm = d_by_size.l1_norm();
    if (!Dune::FloatCmp::eq(d_l1_norm, D_ScalarType(0))) DUNE_THROW(Dune::Exception, d_l1_norm);
    D_ScalarType d_l2_norm = d_by_size.l2_norm();
    if (!Dune::FloatCmp::eq(d_l2_norm, D_ScalarType(0))) DUNE_THROW(Dune::Exception, d_l2_norm);
    D_ScalarType d_sup_norm = d_by_size.sup_norm();
    if (!Dune::FloatCmp::eq(d_sup_norm ,D_ScalarType(0))) DUNE_THROW(Dune::Exception, d_sup_norm);
    ...
    if (!d_by_size_and_value.almost_equal(d_ones)) DUNE_THROW(Dune::Exception, "");
    VectorImp d_added = d_ones.add(d_by_size);
    if (!d_added.almost_equal(d_by_size_and_value)) DUNE_THROW(Dune::Exception, "");
    d_added.iadd(d_by_size);
    if (!d_added.almost_equal(d_ones)) DUNE_THROW(Dune::Exception, "");
    d_ones.sub(d_by_size, d_by_size_and_value);
    if (!d_by_size_and_value.almost_equal(d_ones)) DUNE_THROW(Dune::Exception, "");
    VectorImp d_subtracted = d_ones.sub(d_by_size);
    if (!d_subtracted.almost_equal(d_by_size_and_value)) DUNE_THROW(Dune::Exception, "");
    d_subtracted.isub(d_by_size);
    if (!d_subtracted.almost_equal(d_ones)) DUNE_THROW(Dune::Exception, "");
    // * of the vector as the interface
    ...
    if (!i_almost_equal) DUNE_THROW(Dune::Exception, "");
    i_by_size_and_value.scal(I_ScalarType(0));
    if (!i_by_size_and_value.almost_equal(d_by_size_2)) DUNE_THROW(Dune::Exception, "");
    I_ScalarType i_dot = i_by_size.dot(d_by_size_and_value_2);
    if (!Dune::FloatCmp::eq(i_dot, I_ScalarType(0))) DUNE_THROW(Dune::Exception, i_dot);
    I_ScalarType i_l1_norm = i_by_size.l1_norm();
    if (!Dune::FloatCmp::eq(i_l1_norm, I_ScalarType(0))) DUNE_THROW(Dune::Exception, i_l1_norm);
    I_ScalarType i_l2_norm = i_by_size.l2_norm();
    if (!Dune::FloatCmp::eq(i_l2_norm, I_ScalarType(0))) DUNE_THROW(Dune::Exception, i_l2_norm);
    I_ScalarType i_sup_norm = i_by_size.sup_norm();
    if (!Dune::FloatCmp::eq(i_sup_norm ,I_ScalarType(0))) DUNE_THROW(Dune::Exception, i_sup_norm);
    ...
    if (!i_by_size_and_value.almost_equal(i_ones)) DUNE_THROW(Dune::Exception, "");
    VectorImp i_added = i_ones.add(d_by_size_2);
    if (!i_added.almost_equal(d_by_size_and_value_2)) DUNE_THROW(Dune::Exception, "");
    i_added.iadd(d_by_size_2);
    if (!i_added.almost_equal(i_ones)) DUNE_THROW(Dune::Exception, "");
    i_ones.sub(d_by_size_2, d_by_size_and_value_2);
    if (!i_by_size_and_value.almost_equal(i_ones)) DUNE_THROW(Dune::Exception, "");
    VectorImp i_subtracted = i_ones.sub(d_by_size_2);
    if (!i_subtracted.almost_equal(d_by_size_and_value_2)) DUNE_THROW(Dune::Exception, "");
    i_subtracted.isub(d_by_size_2);
    if (!i_subtracted.almost_equal(i_ones)) DUNE_THROW(Dune::Exception, "");
  } // void fulfills_interface() const
  ...
  void produces_correct_results() const
  {
    ...
    if (sum_operator_iplus != sum_operator_plus || sum_add_1 != sum_add_2
      ...
      DUNE_THROW(Dune::Exception, sum_add_1 << ", " << sum_add_2 << ", " << sum_operator_iplus << ", "
    ...
    if (diff_operator_iminus != diff_operator_minus || diff_sub_1 != diff_sub_2
      ...
      DUNE_THROW(Dune::Exception, diff_sub_1 << ", " << diff_sub_2 << ", " << diff_operator_iminus << ", "
  ...
  : public ::testing::Test
{
  ...
  void fulfills_interface() const
  {
    ...
    if (d_rows != dim) DUNE_THROW(Dune::Exception, d_rows << " vs. " << dim);
    size_t d_cols = d_by_size.cols();
    if (d_cols != dim) DUNE_THROW(Dune::Exception, d_cols << " vs. " << dim);
    ...
    if (result != zeros) DUNE_THROW(Dune::Exception, "check mv");
    ...
    for (size_t ii = 0; ii < rows ; ++ii) {
      for (size_t jj = 0; jj < cols; ++jj) {
        if (FloatCmp::ne(result_axpy.get_entry(ii, jj), ScalarType(0.5)*testmatrix_sparse.get_entry(ii, jj)))
          DUNE_THROW(Dune::Exception, result_axpy.get_entry(ii, jj) << " vs. "
