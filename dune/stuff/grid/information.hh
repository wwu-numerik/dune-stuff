...
unsigned int maxNumberOfNeighbors(const GridPartType& gridPart)
{
  ...
  for (EntityIteratorType entityIterator = gridPart.template begin< 0 >();
       entityIterator != entityIteratorEnd; ++ entityIterator) {
    ...
    for (IntersectionIteratorType intersectionIterator = gridPart.ibegin(entity);
         intersectionIterator != intersectionIteratorEnd; ++ intersectionIterator) {
      ++neighbours;
    } // walk over all neighbors
...
inline std::ostream& operator<<(std::ostream& s, 
		const Dune::Stuff::Grid::Information::Dimensions< T >& d) {
