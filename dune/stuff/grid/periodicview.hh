...
public:
  ...
  PeriodicIntersection(const BaseType& real_intersection, const RealGridViewType& real_grid_view, const std::tuple< bool, EntityPointer, const BaseType* >& periodic_tuple)
    ...
    : public RealGridViewImp::IntersectionIterator
{
  ...
  PeriodicIntersectionIterator(BaseType& real_intersection_iterator, const RealGridViewType& real_grid_view, const EntityType& entity, const std::map< IntersectionIndexType, std::tuple< bool, EntityPointerType, const RealIntersectionType* > >& intersection_map)
    : BaseType(real_intersection_iterator)
    , current_intersection_((real_intersection_iterator == real_grid_view.iend(entity)) ? *real_grid_view.ibegin(entity) : BaseType::operator*(),
                            real_grid_view,
                            intersection_map.at((real_intersection_iterator == real_grid_view.iend(entity)) ? *real_grid_view.ibegin(entity) : BaseType::operator*()).indexInInside)
    , real_grid_view_(real_grid_view)
    , entity_(entity)
  ...
  {
    BaseType::operator++();
    current_intersection_ = Intersection(BaseType::equals(real_grid_view_.iend(entity_)) ? *real_grid_view_.ibegin(entity_) : BaseType::operator*(), real_grid_view_, intersection_map_.at((BaseType::equals(real_grid_view_.iend(entity_)) ? *real_grid_view_.ibegin(entity_) : BaseType::operator*()).indexInInside()));
    return *this;
  }
...
private:
  ...
  const std::map< IntersectionIndexType, std::tuple< bool, EntityPointerType, const RealIntersectionType* > >& intersection_map_;
...
public:
  ...
  enum { conforming = RealGridViewTraits::conforming };
...
public:
    ...
    , periodic_directions_(periodic_directions)
  {
    ...
    for (auto it = real_grid_view_.template begin< 0 >(); it != it_end; ++it) {
      ...
      std::map< IntersectionIndexType, std::tuple< bool, EntityPointerType, const RealIntersectionType* > > intersection_neighbor_map;
        ...
        for (auto i_it = real_grid_view_.ibegin(entity); i_it != i_it_end; ++i_it) {
          ...
          if (intersection.boundary()) {
            ...
            if (is_periodic) {
              ...
              for (auto neighbor_i_it = real_grid_view_.ibegin(*periodic_neighbor); neighbor_i_it != neighbor_i_it_end; ++neighbor_i_it) {
                const auto& outside_intersection = *i_it;
                if (Dune::Stuff::Common::FloatCmp::eq(periodic_neighbor_coords, outside_intersection.geometry().center())) {
                  outside_intersection_pointer = &outside_intersection;
                }
              }
              const size_t index_in_inside = intersection.indexInInside();
              const auto pair = std::make_pair(index_in_inside, std::make_tuple(is_periodic, periodic_neighbor, outside_intersection_pointer));
              intersection_neighbor_map.insert(pair);
            } else {
              intersection_neighbor_map.insert(std::make_pair(intersection.indexInInside(), std::make_tuple(is_periodic, EntityPointerType(entity), &intersection)));
            }
          } else {
            intersection_neighbor_map.insert(std::make_pair(intersection.indexInInside(), std::make_tuple(bool(false), EntityPointerType(entity), &intersection)));
          }
        }
        entity_to_intersection_map_map_.insert( std::make_pair(real_grid_view_.indexSet().index(entity), intersection_neighbor_map));
//      }
    }
  }
  ...
  {
    return IntersectionIterator(real_grid_view_.ibegin(entity), real_grid_view_, entity, entity_to_intersection_map_map_.at(real_grid_view_.indexSet().index(entity)));
  }

  IntersectionIterator
  iend (const typename Codim< 0 >::Entity &entity) const
  {
    return IntersectionIterator(real_grid_view_.iend(entity), real_grid_view_, entity, entity_to_intersection_map_map_.at(real_grid_view_.indexSet().index(entity)));
  }

private:
  std::map< EntityIndexType, std::map< IntersectionIndexType, std::tuple< bool, EntityPointerType, const RealIntersectionType* > > > entity_to_intersection_map_map_;
  const RealGridViewType& real_grid_view_;
...
class PeriodicGridView
    : Dune::Stuff::Common::ConstStorageProvider< PeriodicGridViewImp< RealGridViewImp > >
...
public:
  ...
  PeriodicGridView(const RealGridViewType& real_grid_view, const std::bitset< dimension > periodic_directions = std::bitset< dimension >().set())
