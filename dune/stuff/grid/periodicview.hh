...
public:
  ...
  PeriodicIntersection(const BaseType& real_intersection, const RealGridViewType& real_grid_view, const std::bitset< dimDomain > periodic_directions)
  ...
  {
    if (periodic_)
      return *(Dune::Stuff::Grid::EntityInlevelSearch< RealGridViewType >(*real_grid_view_).operator()(periodic_neighbor_coords_)[0]);
    else
...
public:
  ...
  PeriodicIntersectionIterator(BaseType& real_intersection_iterator, const RealGridViewType& real_grid_view, const std::bitset< dimDomain > periodic_directions, const EntityType& entity)
    : BaseType(real_intersection_iterator)
    , current_intersection_((real_intersection_iterator == real_grid_view.iend(entity)) ? *real_grid_view.ibegin(entity) : BaseType::operator*(), real_grid_view, periodic_directions)
  ...
  {
    BaseType::operator++();
    current_intersection_ = Intersection(BaseType::equals(real_grid_view_.iend(entity_)) ? *real_grid_view_.ibegin(entity_) : BaseType::operator*(), real_grid_view_, periodic_directions_);
    return *this;
  }
...
public:
  PeriodicGridView(const RealGridViewType& real_grid_view, const std::bitset< dimDomain > periodic_directions = std::bitset< dimDomain >().set())
