    ...
    for (auto it = real_grid_view_.template begin< 0 >(); it != it_end; ++it) {
      ...
      for (auto i_it = real_grid_view_.ibegin(entity); i_it != i_it_end; ++i_it) {
        ...
        if (intersection.boundary()) {
          ...
          if (is_periodic) {
            EntityPointerType periodic_neighbor = *(Dune::Stuff::Grid::EntityInlevelSearch< RealGridViewType >(real_grid_view_, new ApplyOn::BoundaryEntities< RealGridViewType >).operator()(std::vector< CoordinateType >(1,  periodic_neighbor_coords))[0]);
