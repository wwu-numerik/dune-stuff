...
public:
                         ...
                         const std::string filename = id()) const
  {
    ...
    std::vector< double > boundaryId = generateBoundaryIdVisualization(gridView);
    vtkwriter.addCellData(boundaryId, "boundaryId");
    const Dune::shared_ptr< const typename Dune::Stuff::Grid::BoundaryInfo::Interface< LeafGridViewType > >
        boundaryInfo = Dune::Stuff::Grid::BoundaryInfo::create< LeafGridViewType >(boundaryInfoType, description);
    // dirichlet values
