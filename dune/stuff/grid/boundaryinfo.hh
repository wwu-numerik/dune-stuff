...
private:
  static std::vector< Common::FieldVector< double, 2 > > get_value_range(const Common::Configuration& cfg,
                                                                         const Common::Configuration& default_cfg,
                                                                         const std::string& id)
  {
...
public:
                                            ...
                                            const std::string sub_name = static_id())
  {
    ...
    return Common::make_unique< ThisType >(default_to_dirichlet,
                                           FunctionProviderType::create(  cfg.has_sub("dirichlet.function")
                                                                        ? cfg.sub("dirichlet.function")
  ...
  FunctionBased(const bool default_is_dirichlet,
                std::shared_ptr< const FunctionType > dirichlet_function,
...
private:
  bool matches(const typename FunctionType::LocalfunctionType& local_function,
               const Common::FieldVector< double, 2 >& value_range,
               const DomainType& xx) const
  {
