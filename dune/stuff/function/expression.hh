  ...
  Expression(const Dune::Stuff::Common::ExtendedParameterTree& paramTree)
  {
    ...
    if (paramTree.hasSub("expression")) {
      ...
      if (actualDimRange_ < 1) {
        ...
        msg << "Error in " << id() << ": subtree 'expression' does not have any keys '0', '1', ... in the following Dune::Parametertree" << std::endl;
      ...
      if (expressions.size() > 1) {
        ...
        std::stringstream msg;
        msg << "Error in " << id() << ": vector 'expression' does not have any entries in the following Dune::Parametertree" << std::endl;
        paramTree.report(msg);
        DUNE_THROW(Dune::InvalidStateException, msg.str());
      }
    } else { // if (paramTree.hasSub("expression"))
      std::stringstream msg;
      msg << "Error in " << id() << ": there is neither a subtree nor a vector called 'expression' in the following Dune::Parametertree" << std::endl;
      paramTree.report(msg);
      DUNE_THROW(Dune::InvalidStateException, msg.str());
    } // if (paramTree.hasSub("expression"))
    // set up
