...
public:
  ...
  FunctionCheckerboardBase(const DomainType _lowerLeft,
  ...
  virtual void evaluate(const DomainType& x, RangeType& ret) const
  {
    ...
    for (int d = 0; d < dimDomain; ++d)
    {
      ...
      whichPartition.push_back(std::min(size_t(std::floor(numElements_[d]*((x[d] - lowerLeft_[d])/(upperRight_[d] - lowerLeft_[d])))),
                                        numElements_[d] - 1));
    }
    size_t subdomain = 0;
    if (dimDomain == 1)
      ...
      subdomain = whichPartition[0]
                  + whichPartition[1]*numElements_[0]
...
public:
  ...
  static ThisType* create(const DSC::ExtendedParameterTree description)
  {
    ...
    const std::vector< RangeFieldType > rangeFieldTypeValues = description.getVector("values", RangeFieldType(1), subdomains);
