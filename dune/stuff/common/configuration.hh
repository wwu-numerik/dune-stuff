...
public:
  ...
  explicit Configuration(const T /*record_defaults*/ = internal::configuration_record_defaults,
                         ...
                         const typename std::enable_if<!std::is_same<T,const char*>::value, bool>::type log_on_exit = internal::configuration_log_on_exit,
  ...
  Configuration(const std::vector< std::string > keys,
                         ...
                         const bool log_on_exit = internal::configuration_log_on_exit,
                         const std::string logfile = internal::configuration_logfile)
   : Configuration(keys, make_string_sequence(values_in.begin(), values_in.end()), record_defaults, warn_on_default_access, log_on_exit, logfile)
  {}
  ...
  typename internal::Typer<T>::type get(std::string key,
        const size_t size,
        const size_t cols = 0,
  ...
  template< class T, class Validator = ValidateAny< typename internal::Typer< T >::type > >
  typename internal::Typer<T>::type get(std::string key,
        const ValidatorInterface< typename internal::Typer< T >::type, Validator >& validator = ValidateAny< typename internal::Typer<T>::type >()) const
  {
    if (!has_key(key))
      DUNE_THROW(Exceptions::configuration_error, "Configuration does not have this key and there was no default value provided");
  ...
  template< typename T, class Validator = ValidateAny< typename internal::Typer< T >::type > >
  std::vector< typename internal::Typer<T>::type > getList(std::string key,
                                                           T def = T(),
...
#define DSC_CONFIG_GETB(key,def,use_logger) \
  DSC_CONFIG.get(key,def, use_logger)
