...
void Profiler::outputTimingsAll(std::ostream& out) const
{
  ...
  for (const auto& section : datamaps_[0]) {
    out << csv_sep << section.first << "_avg_usr" << csv_sep << section.first << "_sum_usr"
        << csv_sep << section.first << "_avg_wall" << csv_sep << section.first << "_sum_wall";
  }
  int i = 0;
  ...
  for (const auto& datamap : datamaps_) {
    ...
    for (const auto& section : datamap) {
      ...
      out << csv_sep << usr_sum * weight << csv_sep << usr_sum
          << csv_sep << wall_sum * weight << csv_sep << wall_sum;
    }
  }
  out << std::endl;
}
