...
TimingData::DeltaType TimingData::delta() const
{
  ...
  const auto cast = [ = ](double var) {
    return static_cast<typename TimingData::DeltaType::value_type>(var);
  };
  return {{
      cast((elapsed.user + elapsed.system) * scale / double(threadManager().current_threads())),
