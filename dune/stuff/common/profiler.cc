...
boost::timer::nanosecond_type TimingData::delta() const
{
    return (timer_->elapsed().user + timer_->elapsed().system)
            / boost::timer::nanosecond_type(1e6);
}
...
void Profiler::outputTimingsAll(std::ostream& out) const
{
  ...
  for (const auto& section : datamaps_[0]) {
    stash << csv_sep << section.first << "_avg_usr" << csv_sep << section.first << "_max_usr"
        << csv_sep << section.first << "_avg_wall" << csv_sep << section.first << "_max_wall";
  }
  int i = 0;
  for (const auto& datamap : datamaps_) {
    stash << std::endl << i++;
    for (const auto& section : datamap) {
      ...
      stash << csv_sep << usr_sum * weight << csv_sep << usr_max
          << csv_sep << wall_sum * weight << csv_sep << wall_max;
    }
  }
  stash << std::endl;
  if (comm.rank() == 0) 
