...
{
public:
  bool operator()(const T& value) const { return asImp()(value); }

  std::string msg() const { return asImp().msg(); }
  void print(std::ostream& out) const { out << asImp().msg(); }
protected:
  inline const Derived& asImp() const { return static_cast<const Derived&>(*this); }

  inline Derived& asImp() { return static_cast<Derived&>(*this); }
};
...
public:
  ...
  std::string msg() const { return "ValidateAny: all values should be valid... \n\n"; }
};
...
public:
  ValidateInList(const ListType& valid_list) : valid_list_(valid_list) {}
...
{
public:
  ValidateInverse(const Validator validator = Validator()) : validator_(validator) {}
  ValidateInverse(const T arg) : validator_(Validator(arg)) {}
  inline bool operator()(const T& val) const { return !validator_(val); }

  std::string msg() const { return "Inverse condition failed: " + validator_.msg(); }
private:
...
{
public:
  ValidateInterval(const T& min, const T& max) : min_(min), max_(max) {}
  ...
  std::string msg() const { return "given value was invalid: value not in given interval"; }
private:
  const T min_;
  const T max_;
};
...
{
  static std::string value() { return "Dune::Stuff::Common::Parameter::ValidateAny<T>"; }
};
