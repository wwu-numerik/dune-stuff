...
/* forward to use tokenize in the get_matrix_from_string and get_vector_from_string methods below (only tokenize<std::string>
  ...
  static VectorType get_vector_from_string(std::string vector_str, const size_t size)
  {
    ...
    if (vector_str.substr(0, 1) == "[" && vector_str.substr(vector_str.size() - 1, 1) == "]") {
      ...
      if (size > 0 && tokens.size() < size)
        DUNE_THROW(Exceptions::configuration_error,
                   "Vector (see below) has only " << tokens.size()
...
{
public:
  static inline bool fromString(const std::string ss,
                                const size_t UNUSED_UNLESS_DEBUG(rows) = 0,
                                const size_t UNUSED_UNLESS_DEBUG(cols) = 0)
  {
    assert(rows == 0);
...
template< class S > \
class Choose< MatrixType < S > > \
  : ChooseBase < MatrixType < S > >\
{ \
...
class Choose< FieldMatrixType< S, ROWS, COLS > >  \
    : ChooseBase < FieldMatrixType< S, ROWS, COLS > >\
{ \
  ...
  static inline FieldMatrixType< S, ROWS, COLS > fromString(const std::string s, const size_t rows = 0, const size_t cols = 0) { \
    if ((rows > 0 && rows != ROWS) || (cols > 0 && cols != COLS)) \
      DUNE_THROW(Exceptions::configuration_error, \
                 "You requested a '" << getTypename( FieldMatrixType < S, ROWS, COLS >()) << "' with a size of " \
                 << rows << "x" << cols << " but this type of matrix can not have any size other than " \
                 << ROWS << "x" << COLS << "!"); \
...
template< class S > \
class Choose< VectorType< S > > \
  : ChooseBase< VectorType< S > > \
{ \
...
class Choose< FieldVectorType< S, SIZE > > \
  : ChooseBase< FieldVectorType< S, SIZE > > \
{ \
...
template< class InType > inline std::string toString(const InType& s) {
                             ...
                             const boost::algorithm::token_compress_mode_type mode /*= boost::algorithm::token_compress_off*/ )
{
    ...
    boost::algorithm::split( strings, msg, boost::algorithm::is_any_of(separators),
                             mode );
