...
#define DSC_has_typedef_initialize_once(tpdef) \
  ...
  \
    ...
    struct helper { static const bool value = false; }; \
  \
    template< typename TT_local > \
    struct helper< TT_local, typename void_< typename TT_local:: tpdef >::type > { static const bool value = true; }; \
  \
...
\endcode
  */
#define DSC_has_typedef(tpdf) \
  DSC_has_typedef_helper_ ## tpdf
...
#define DSC_has_static_member_initialize_once(mmbr) \
  ...
  { \
    template< class TT_local, class = typename std::enable_if< !std::is_member_pointer< decltype(&TT_local:: mmbr) >::value>::type > \
...
\endcode
  */
#define DSC_has_static_member(mmbr) \
  DSC_has_static_member_helper_ ## mmbr
