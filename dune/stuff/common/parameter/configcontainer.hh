...
private:
  typedef std::map<std::string, std::set<Request> >
    ...
    T get_valid_value(std::string name, T def,
...
// Constructors and destructors
    ...
    //! warning output = true, record_defaults = false, logdir_ = ???, tree_[keys[ii]] = values[ii] for 0 <= ii <= keys.size()
    template< class T >
    ...
    template< typename T >
    T get(const std::string name, T def, Request req, bool useDbgStream = true, const size_t size = 0, const size_t cols = 0) {
    ...
    template< typename T, class Validator >
    T get(const std::string name, T def, const ValidatorInterface< T, Validator >& validator,
          Request req, bool useDbgStream = true, const size_t size = 0, const size_t cols = 0) {
    ...
    //! get variation with default value, validation
    template< typename T , class Validator >
    T get(const std::string name, T def, const ValidatorInterface< T, Validator >& validator, bool useDbgStream = true, const size_t size = 0, const size_t cols = 0)
    {
      Request req(-1, std::string(), name,
                  Dune::Stuff::Common::toString(def),
                  Dune::Stuff::Common::getTypename(validator));
    ...
    T get(const std::string name, const ValidatorInterface< T, Validator >& validator, size_t size = 0, size_t cols = 0, bool useDbgStream = true) const {
      if (!has_key(name))
        DUNE_THROW(InvalidParameter, "ConfigContainer does not have this key and there was no default value provided");
    ...
    template< class T , class Validator>
    T get(const std::string name, T def, const ValidatorInterface< T, Validator >& validator, size_t size = 0, size_t cols = 0, bool useDbgStream = true) const{
      return get_valid_value(name, def, validator, size, cols);
    }
